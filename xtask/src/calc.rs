...
fn size_calc(value: &str) -> Result<()> {
    ...
    println!("{} = {} (0x{:X}) = {} = {} 4K pages",
        value, bytes, bytes, bs.to_string_as(true), (bytes + 4095) / 4096);
...
fn addr_calc(value: &str) -> Result<()> {
    ...
    let canonical = if addr & (1 << 47) != 0 { (addr >> 48) == 0xFFFF } else { (addr >> 48) == 0 };
    let align_4k = (addr & 0xFFF) == 0;
    let align_2m = (addr & 0x1FFFFF) == 0;
    
    println!("0x{:016X} ({})", addr, addr);
    println!("  4K: page={} off=0x{:03X} | PT[{}][{}][{}][{}]", page_4k, off_4k, pml4, pdpt, pd, pt);
    println!("  canonical={} 4K-aligned={} 2M-aligned={}", canonical, align_4k, align_2m);
...
fn buddy_calc(size: &str, page_size: u64) -> Result<()> {
    ...
    println!("{} ({}) -> order={} (2^{}={} pages, {} actual, {:.0}% waste)",
        bytes, bs.to_string_as(true), order, order, actual_pages, 
        ByteSize::b(actual_bytes).to_string_as(true), waste_pct);
...
fn bits_calc(value: &str) -> Result<()> {
    ...
    let is_pow2 = num != 0 && (num & (num - 1)) == 0;
    
    println!("{} = 0x{:X} = 0b{:b}", num, num, num);
    println!("  bits set: {} | leading 0s: {} | trailing 0s: {}{}",
        set_bits.len(), num.leading_zeros(), 
        if num == 0 { 64 } else { num.trailing_zeros() },
        if is_pow2 { format!(" | 2^{}", num.trailing_zeros()) } else { String::new() });
    
    if !set_bits.is_empty() && set_bits.len() <= 16 {
        let positions: String = set_bits.iter().rev().map(|b| b.to_string()).collect::<Vec<_>>().join(",");
...
fn align_calc(value: &str, to: u64) -> Result<()> {
    ...
    println!("0x{:X} align({}) -> down=0x{:X} up=0x{:X} off={} aligned={}",
        num, to, down, up, off, num == down);
...
fn eval_expr(expr: &str) -> Result<()> {
    ...
    if let Some((a, b)) = expr.split_once('+') {
        ...
        println!("  {} + {} = {} (0x{:X})", a, b, result, result);
        return Ok(());
    }
    if let Some((a, b)) = expr.split_once('-') {
        let a = parse_size(a.trim())?;
    ...
    if let Some((a, b)) = expr.split_once('/') {
        ...
        let result = a / b;
        let remainder = a % b;
        println!("  {} / {} = {} remainder {} (0x{:X})", a, b, result, remainder, result);
        return Ok(());
    }
    if let Some((a, b)) = expr.split_once("<<") {
        let a = parse_size(a.trim())?;
    ...
    if let Some((a, b)) = expr.split_once('&') {
        ...
        let result = a & b;
        println!("  0x{:X} & 0x{:X} = {} (0x{:X})", a, b, result, result);
        return Ok(());
    }
    if let Some((a, b)) = expr.split_once('|') {
        let a = parse_addr(a.trim())?;
    ...
    if expr.starts_with('~') {
        ...
        let result = !a;
        println!("  ~0x{:X} = {} (0x{:X})", a, result, result);
        return Ok(());
    }
    
    // Just parse and display
    let val = parse_size(expr)?;
    println!("  {} = {} (0x{:X})", expr, val, val);
