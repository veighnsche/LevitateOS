...
- `dump.erofs --path /usr/local/bin/stage-01-live-boot.sh .artifacts/out/<DistroDir>/s00-build/s00-filesystem.erofs`
...
- `.artifacts/out/<DistroDir>/sNN-<stage-name>/sNN-filesystem.erofs`
- `.artifacts/out/<DistroDir>/sNN-<stage-name>/sNN-initramfs-live.cpio.gz`
- `.artifacts/out/<DistroDir>/sNN-<stage-name>/sNN-overlayfs.erofs`
- `.artifacts/out/<DistroDir>/sNN-<stage-name>/sNN-initramfs-installed.img`
- `.artifacts/out/<DistroDir>/sNN-<stage-name>/*.iso`
- `.artifacts/out/<DistroDir>/sNN-<stage-name>/*.sha512`
...
### Split Stage Artifact Policy (Strict)
- Every stage writes non-kernel outputs to its own directory under `.artifacts/out/<DistroDir>/` using numbered names:
- `s00-build`, `s01-boot`, `s02-live-tools`, `s03-install`, `s04-login-gate`, `s05-harness`, `s06-runtime`, `s07-update`, `s08-package`.
- Non-kernel artifacts must be stage-scoped and must never be read from another stage directory.
- Minimum protected examples for this rule: `sNN-filesystem.erofs`, `sNN-initramfs-live.cpio.gz`, `sNN-overlayfs.erofs`, stage ISO, and stage checksums.
- Stage 01 must not consume Stage 00 non-kernel files; Stage 00 and Stage 01 may both consume the same shared kernel artifacts.
- Allowed shared artifacts are kernel-only (as defined in Kernel Boundary Policy). Kernel artifacts remain outside stage folders in their canonical kernel paths.
- Prohibited patterns: cross-stage symlinks, alias paths, "reuse by copy" shortcuts, or manual post-build file surgery to make one stage look like another.
- Reproducibility requirement: a stage build command must succeed from code alone when kernel artifacts already exist, even if all non-kernel stage folders are empty.

### Stage Envelope Policy (Nothing More, Nothing Less)
- Each stage ISO/rootfs/initramfs/overlay must contain exactly the payload required for that stage boundary.
- "Passes stage N" is invalid if the artifact also carries payload that belongs to a later stage (stage leakage).
- "Passes stage N" is invalid if required stage-N payload is missing (stage underfill).
- For each stage, declare required paths and forbidden paths in `distro-contract`; treat both missing-required and present-forbidden as conformance failures.
- `distro-builder` must assemble stage artifacts from stage-specific declarations/manifests, not from broad shared roots with implicit carry-over.

### Stage Rootfs Assembly Policy (Incremental Producer Model)
- Stage rootfs assembly is incremental by stage and generated by stage producers in `distro-builder`, not by broad tree copy + filtering.
- Do not shape stage rootfs by deleting from a larger tree after the fact.
- Prohibited for stage shaping: prune lists, exclude lists, post-copy deletion passes, or any subtractive filtering strategy.
- Stage progression must be additive and explicit:
- `s00` contains only `00Build` minimum payload.
- `s01` contains `s00` baseline plus `01Boot` additions required for live boot boundary.
- Apply the same pattern for all later stages (`s02..s08`): add only that stage's required payload.
- `distro-variants/*/NNStage.toml` files are stage endpoints for wiring, but their contents must be OS-policy deltas only.
- OS-policy manifest data may declare distro differences (for example overlay flavor, init style, profile overlay path, branding), not per-stage file inclusion/exclusion lists.
- Per-stage payload composition rules and invariants belong in shared code/contracts (`distro-builder`, `distro-contract`) as a single source of truth.
...
- `rootfs_erofs` (e.g. `.artifacts/out/<DistroDir>/sNN-<stage-name>/sNN-filesystem.erofs`)
- `initramfs` (e.g. `.artifacts/out/<DistroDir>/sNN-<stage-name>/sNN-initramfs-live.cpio.gz`)
- `install_initramfs` (e.g. `.artifacts/out/<DistroDir>/sNN-<stage-name>/sNN-initramfs-installed.img`)
- `iso` (e.g. `.artifacts/out/<DistroDir>/sNN-<stage-name>/*.iso`)
- `iso_checksum` (e.g. `.artifacts/out/<DistroDir>/sNN-<stage-name>/*.sha512` or `.artifacts/out/<DistroDir>/sNN-<stage-name>/*.iso.sha512`)
